        ...
        if self.idle == 0:
            for x in range(self.x - 2, self.x + 3):
                for y in range(self.y - 2, self.y + 3):
                    if not (x in range(self.x - 1, self.x + 2) and y in range(self.y - 1,
                                                                              self.y + 2)) and world.get_obj(x,
                                                                                                             y) and world.get_objname(
                            x, y)[:4] == "Tree" and not self.output and hasp:
                        self.output.append(world.get_obj(x, y).cut(world, False))
                        break
            self.idle = 60
        else:
...
class FTab(object):
    ...
    def __init__(self):
        self.menu = [Buyers.ObjBuyer(AutoChopper, 1000), Buyers.ObjBuyer(PaperMill, 2000),
                     Buyers.VBuyer(ChopMaster9000, 3000),
                     Buyers.ObjBuyer(AutoChopperPlus, 5000), Buyers.VBuyer(ChopMaster1000, 1000),
                     Buyers.UnOwnObjBuyer(Tree, 1000), Buyers.UnOwnObjBuyer(SpTree, 5000)]
