...
class FFBuyer(Buyers.ObjBuyer):
    ...
    def buy(self, world, tx, ty, p):
        if all([world.is_placeable(tx, ty), world.is_placeable(tx + 1, ty), world.is_placeable(tx, ty + 1),
                world.is_placeable(tx + 1, ty + 1)]):
            ff = FishFarm(tx, ty, p)
            world.spawn_obj(ff)
...
class FisheryTab(object):
    ...
    def __init__(self):
        self.menu = [FloatBuyer(Fisher, 500), FFBuyer(FishFarm, 500), Buyers.VBuyer(Vehicles.Boat, 500),
                     Buyers.BridgeBuyer()]
