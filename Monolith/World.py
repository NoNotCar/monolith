...
class World(object):
    ...
    def scrollrender(self, screen):
        ...
        for ent in [en for en in self.ents if not en.hidden]:
            if abs(ent.x - sx) < 7 and abs(ent.y - sy) < 7:
                sscreen.blit(ent.get_img(),
                             (ent.x * 32 - asx + int(round(ent.xoff)), ent.y * 32 - asy + int(round(ent.yoff))))
        # Player Rendering
        ...
        screen.blit(Img.pfont2.render("%g" % (ply.rpsupply / 1000.0) + "kW", True,
                                      (100, 255, 100) if ply.rpsupply > 0 else (255, 0, 0) if ply.rpsupply < 0 else (
                                      255, 255, 255)), (16, 368))
        screen.blit(Img.pfont2.render("%g" % (round(sum([ps.stored for ps in ply.pstorage]) / 60000.0, 1)) + "/" +
                                      "%g" % (round(sum([ps.maxS for ps in ply.pstorage]) / 60000.0, 1)) + "kJ", True,
                                      (100, 255, 100)), (128, 368))
        for obj3 in is3ds:
    ...
    def is_clear(self, x, y, player=False, boat=False):
        ...
        if boat and not self.get_terr(x, y).iswasser:
            return False
        return not (
        self.get_obj(x, y) and (self.get_obj(x, y).solid or (player and not self.get_obj(x, y).playerenter))) and not (
        self.get_ent(x, y) and self.get_ent(x, y).solid)
    ...
    def make_map(self):
        ...
        for x in range(32):
            for y in range(32):
                pygame.draw.rect(mmap, self.get_terr(x * self.size[0] // 32, y * self.size[1] // 32).mcol,
        ...
        pygame.draw.rect(mmap, (255, 0, 0),
                         pygame.Rect(self.player.x * 64 // self.size[0] - 1, self.player.y * 64 // self.size[1] - 1, 4,
                                     4))
        return mmap
