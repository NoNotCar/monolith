            ...
            for mach in self.machines:
                if mach.output:
                    ...
                    for bot in world.ents:
                        if "Robotic" in bot.types and not bot.moving and abs(bot.x - self.x) + abs(
                                        bot.y - self.y) == 1 and bot.load(ent):
                            mach.output.pop(0)
                            self.wait = 30
                            break
        else:
    ...
    def update(self, world):
        if not self.wait:
            ...
            if self.buffer:
                for bot in world.ents:
                    if "Robotic" in bot.types and not bot.moving and abs(bot.x - self.x) + abs(
                                    bot.y - self.y) == 1 and bot.load(self.buffer[0]):
                        self.buffer.pop(0)
                        self.wait = 30
                        break
        else:
...
class RobInput(Object.OObject):
    ...
    def update(self, world):
        if not self.wait:
            ...
            for mach in self.machines:
                for bot in world.ents:
                    if "Robotic" in bot.types and not bot.moving and len(bot.output) and abs(bot.x - self.x) + abs(
                                    bot.y - self.y) == 1:
...
class RobotCategory(object):
    ...
    def __init__(self):
        self.menu = [RobotBuyer(Robot, 300), RobotBuyer(FarmBot, 1000), RobotBuyer(AmphiBot, 1000),
                     RobotBuyer(ChopBot, 1000),
                     Buyers.RotObjBuyer(Director, 100), Buyers.RotObjBuyer(RevDirector, 100),
                     Buyers.RotObjBuyer(DiaDirector, 300),
                     Buyers.ObjBuyer(UnloadDir, 100), Buyers.ObjBuyer(LoadDir, 100),
                     Buyers.RotObjBuyer(LoadDirRot, 300), Buyers.RotObjBuyer(WeakDirector, 100),
                     Buyers.ObjBuyer(RobOutput, 100), Buyers.ObjBuyer(RobOutputGold, 500),
                     Buyers.ObjBuyer(RobInput, 100)]
