    ...
    def update(self, world):
        if not (self.owner.estop or self.stopped):
            ...
            if self.ent and (self.ent.x != self.x or self.ent.y != self.y):
                self.ent = None
            if self.hasp and self.ent and self.ent.move(Mech.dirconv[self.dir][0], Mech.dirconv[self.dir][1],
                                                        self.speed, world):
                self.ent = None
        ...
        if self.hasp:
            ...
            for mach in self.machines:
                if mach.output:
                    ...
                    if world.is_clear(tx, ty) and world.get_objname(tx, ty) in ["Conveyor", "SlowConveyor",
                                                                                "FastConveyor"]:
                        world.spawn_ent(ent)
...
class PowerCategory(object):
    ...
    def __init__(self):
        self.menu = [Buyers.ObjBuyer(SolarPanel, 1000), Buyers.ObjBuyer(Generator1, 200), Buyers.ObjBuyer(Battery, 500),
                     Buyers.RotObjBuyer(PushPlate, 200), Buyers.RotObjBuyer(PowerOutput, 500)]
